...
def cart_text(cart: Dict[int, dict]):
    ...
    for item in cart.values():
        ...
        lines.append(f"• <b>{html.escape(item['name'])}</b>\n  {html.escape(item['package_label'])} × {item['quantity']} = {subtotal} ₽")
...
async def shop(message: Message, state: FSMContext):
    ...
    if 'cart' not in data: await state.update_data(cart={})
...
async def cb_prod(c: CallbackQuery):
    ...
    if not p or not p['active'] or not p['price']:
...
async def fulfillment(message: Message, state: FSMContext):
    ...
    await state.update_data(fulfillment=value); await state.set_state(Checkout.cafe)
...
def order_chat_text(order, items):
    ...
    for i in items: lines.append(f"• {html.escape(i['product_name'])}\n  {html.escape(i['package_label'])} × {i['quantity']} = {i['price']*i['quantity']} ₽")
    lines += ['', f"💰 <b>Итого: {order['total']} ₽</b>", '', '⏳ Статус: новый заказ']
    return '\n'.join(lines)
